...
function handlePrediction(data) {
  ...
  if (data.word) {
    ...
    detectionConfidence.style.color = getComputedStyle(
...
async function startCamera() {
  try {
    ...
    showToast(
      "❌ Gagal mengakses kamera. Pastikan webcam tersedia.",
      "error",
      5000,
    );
  }
  ...
  hands.forEach((landmarks, handIdx) => {
    ...
    for (const [a, b] of HAND_CONNECTIONS) {
      if (landmarks[a] && landmarks[b]) {
        ctx.beginPath();
        // Video tampil ter-mirror (CSS scaleX(-1)), jadi koordinat X landmark
        // juga harus di-mirror: x = (1 - lm.x) agar cocok dengan tangan di layar
        ctx.moveTo(
    ...
    for (let i = 0; i < landmarks.length; i++) {
      ...
      const x = (1 - lm.x) * canvas.width; // mirror X agar cocok dengan video
...
async function init() {
  ...
  try {
    ...
    if (data.model_loaded) {
      const version = data.model_version || 1;
      const typeLabel =
        version >= 2 ? "Motion 2-Tangan" : "Statis 1-Tangan (lama)";
      modelStatusText.textContent = `Model v${version} · ${typeLabel} · ${data.words.length} kata`;
      modelStatus.classList.add("active");

      // Peringatan jika model masih v1 (belum mendukung motion)
      if (version < 2) {
        showToast(
          "⚠️ Model masih v1 (statis). Rekam data baru & latih ulang untuk deteksi 2 tangan + motion!",
          "error",
          6000,
        );
      }
